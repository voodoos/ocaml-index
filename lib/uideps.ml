...
module Shape_full_reduce = Shape_reduce.Make_reduce (struct
  ...
  let load_index comp_unit filename =
    ...
    | exception Not_found ->
      ...
      @@ String.concat "; " (Load_path.get_paths ()); None
  ...
  let try_load ~unit_name () =
    ...
    *)
    ...
    let shape = match Hashtbl.find_opt index_shapes lib_name with
...
let generate_one ~root ~build_path:_ cmt_path =
  ...
  | _ -> Log.error "No implementation in %s\n%!" cmt_path; None
...
let generate ~root ~output_file ~build_path cmt =
    ...
    Option.iter (fun (partials, unreduced, cu_shape, load_path) ->
